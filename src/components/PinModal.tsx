...
export default function PinModal({ 
  ...
  const processPinEntry = useCallback(async (pin: string) => {
    setIsVerifying(true);
    const isCorrect = await onVerify(pin);
  ...
  }, [onVerify, onAuthenticationSuccess, failedAttempts]);
  ...
  }, [currentPinAttempt, hasAuthError, lockoutTimeLeft, isVerifying, processPinEntry]);
      ...
      >
        ...
        <div className={theme.headerWrapper}>
          ...
          <h2 className={theme.typography.title}>
            {lockoutTimeLeft > 0 ? 'Güvenlik Kilidi' : (isVerifying ? 'Doğrulanıyor...' : 'Giriş Yapın')}
          </h2>
        ...
        {/* SECURE NUMBER PAD */}
        <div className={`transition-opacity duration-300 ${(lockoutTimeLeft > 0 || isVerifying) ? theme.animations.lockout : 'opacity-100'}`}>
