              ...
              <div className="flex items-center gap-2">
                ...
                {settings.displayConfig.showInstagram && (
                  <div 
                    ...
                    className={`text-stone-400 transition-all active:scale-75 cursor-pointer ${isAdmin ? 'hover:text-pink-600 ring-2 ring-transparent hover:ring-pink-100 rounded p-0.5 -m-0.5' : 'hover:text-pink-600'}`}
                ...
                {settings.displayConfig.showWhatsapp && (
                  <div className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 bg-stone-900 text-white rounded-md transition-all active:scale-95 shadow-sm hover:bg-black cursor-pointer" onClick={handleWhatsAppAction}>
                    ...
                    <span 
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), e.currentTarget.blur())}
                      className={`text-[9px] sm:text-[10px] font-medium tracking-tight outline-none ${isAdmin && isInlineEnabled ? 'min-w-[50px] focus:bg-white/10' : ''}`}
                    >
