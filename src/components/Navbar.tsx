...
export default function Navbar({ settings, isAdmin, updateSetting }: { settings: CompanySettings, isAdmin?: boolean, updateSetting?: (key: keyof CompanySettings, value: string) => void }) {
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex justify-between py-2 items-center gap-4">
          ...
          <div className="flex items-center gap-2 select-none shrink-0 cursor-default">
            <span 
              onClick={(e) => handleEdit(e, 'logoEmoji', 'Logo Emojisi', settings.logoEmoji || DEFAULT_COMPANY.logoEmoji)}
              className={`${n.logo.emojiSize} ${isAdmin ? 'cursor-pointer hover:opacity-70 transition-opacity' : ''}`}
              title={isAdmin ? "Logo Değiştir" : ""}
            >
              {settings.logoEmoji || DEFAULT_COMPANY.logoEmoji}
            </span>
            <div className="flex flex-col">
              <div 
                onClick={(e) => handleEdit(e, 'title', 'Marka Adı', settings.title)}
                className={`${n.logo.nameSize} ${n.logo.nameWeight} text-stone-900 leading-none tracking-tight ${isAdmin ? 'cursor-pointer hover:opacity-70 transition-opacity' : ''}`}
                title={isAdmin ? "Marka Adı Değiştir" : ""}
              >
                {settings.title}
              </div>
              <div 
                onClick={(e) => handleEdit(e, 'subtitle', 'Alt Başlık', settings.subtitle)}
                className={`${n.logo.taglineSize} ${n.logo.taglineColor} font-medium ${isAdmin ? 'cursor-pointer hover:opacity-70 transition-opacity' : ''}`}
                title={isAdmin ? "Alt Başlık Değiştir" : ""}
              >
                {settings.subtitle}
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-0 shrink-0 min-w-0">
            ...
            <a
              ...
              onClick={(e) => handleEdit(e, 'address', 'Firma Adresi', settings.address)}
              className={`${n.address.size} ${n.address.color} ${n.address.hoverColor} transition-colors whitespace-nowrap order-2 sm:order-1 truncate max-w-[120px] sm:max-w-none ${isAdmin ? 'cursor-pointer' : ''}`}
              title={isAdmin ? "Adresi Düzenle" : settings.address}
            >
              {settings.address}
            </a>
            ...
            <div className="flex items-center gap-2 sm:gap-3 order-1 sm:order-3">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                onClick={(e) => handleEdit(e, 'instagram', 'Instagram Linki', settings.instagram)}
                className={`${n.contact.instaColor} ${n.contact.instaHover} transition-all active:scale-90 ${isAdmin ? 'cursor-pointer' : ''}`}
                aria-label="Instagram Profilimiz"
                title={isAdmin ? "Instagram Linki Düzenle" : ""}
              >
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                onClick={(e) => handleEdit(e, 'whatsapp', 'WhatsApp Numarası', settings.whatsapp)}
                className={`flex items-center gap-1.5 px-2 py-1 ${n.contact.whatsappBg} ${n.contact.whatsappHover} ${n.contact.whatsappRounded} text-white transition-all active:scale-95 shadow-md ${isAdmin ? 'cursor-pointer' : ''}`}
                aria-label="Bizimle WhatsApp üzerinden iletişime geçin"
                title={isAdmin ? "WhatsApp Numarası Düzenle" : ""}
              >
