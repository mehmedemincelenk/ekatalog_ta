...
// Mock Supabase with conditional logic for failure simulation
const mockSingle = vi.fn((pin?: string) => {
  if (!pin || pin === 'FAIL') return Promise.resolve({ data: null, error: { message: 'Invalid' } });
      ...
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          eq: vi.fn((_col: string, val: string) => ({
            single: () => mockSingle(val) // Use the value passed to .eq('admin_pin', val)
          }))
        }))
      }))
    }))
